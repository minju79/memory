...
| 옵션 | 설명 |
|---|---|
| `--region 광주` | 제목·기관에 이 말이 있는 것만. 축에 걸렸는데 지역명이 없으면 버리지 않고 '지역 미명시' 로 따로 모읍니다 (중앙부처 자료 대비) |
| `--axis 반도체AI` | 이 투자축만 (반복 가능) |
| `--include-general` | 4개 축 밖의 일반 개발 소식까지 (기본은 축만) |
...
레이더가 넓으면 매일 쌓이기만 하고 아무것도 안 남습니다. 지금 광주를 실제로 움직이는 축만 봅니다.

| 축 | 담는 것 |
|---|---|
| `통합` | 광주·전남 행정통합, 통합특별시 |
| `반도체AI` | 반도체 특화단지, AI 데이터센터, 첨단3지구 |
| `토지허가` | 토지거래허가구역 지정·해제 — **호재가 아니라 규제**라서 빨강 |
| `군공항` | 군공항 이전, 종전부지, 이전 후보지 |

카드마다 두 가지가 배지로 붙습니다.

- **개발단계 0~7** — 배지 + 8칸 게이지
  3단계(공식지정·고시)가 분기점이라 그 전까지는 테두리로만 표시됩니다. 애매하면 한 단계 낮춰 잡는 것이 규칙입니다.
- **출처 등급** — 초록 `공식자료` / 회색 `언론보도`
  "뉴스 옮기는 사람"이 아니라 "공식 자료로 확인해주는 사람"이라는 차이를 카드에서 보이게 합니다.
